...
class Product:
    ...
    def display_product_info(self):
        print(f"Product: {self.name}, Price: ${self.price:.2f}, Stock: {self.quantity_in_stock}")

    def update_stock(self, new_quantity):
        self.quantity_in_stock = new_quantity
        print(f"Stock for {self.name} updated to {self.quantity_in_stock} units.")
...
# Additional test: Update stock for item2
item2.update_stock(150)
item2.display_product_info()
new_total_value_item2 = item2.calculate_total_value()
print(f"New total stock value for {item2.name}: ${new_total_value_item2:.2f}")
